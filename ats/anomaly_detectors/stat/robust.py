...
class _COMNHARAnomalyDetector(AnomalyDetector):
    ...
    def apply(self, data, *args, **kwargs):
        ...
        if isinstance(data, pd.DataFrame):
            ...
            # List of multivariate time series not supported
            for _dataframe in data:
                if len(_dataframe.columns) > 1:
                    raise NotImplementedError('Cannot work on list of multivariate time series yet')
            timeseries_df = list_of_timeseries_df_to_timeseries_df(data)
        else:
        ...
        if timeseries_df.index.name != "timestamp":
        ...
        results_dataframe = timeseries_df.copy()
        results_dataframe["anomaly"] = False
        for variable_name in results_dataframe.columns:
        ...
        for method, method_results in detections.items():
            for _ , method_result in method_results.iterrows():
                ...
                variable_name = results_dataframe.columns[variable_index]
                time_index = method_result["row"]
                timestamp = results_dataframe.index[time_index]
        ...
        # De-assemble if required
        if isinstance(data, list):
            # Drop the "global" anomaly column
            results_dataframe.drop('anomaly', axis=1, inplace=True)

            results_dataframe = timeseries_df_to_list_of_timeseries_df(results_dataframe, anomaly_labels=True)
            #print(results_dataframe)
...
class COMAnomalyDetector(_COMNHARAnomalyDetector):
    ...
    def __init__(self, fq=2 * np.pi / 30, fw=2 * np.pi / 7, trend=2):
        super().__init__(fq=fq, fw=fw, trend=trend, methods=('COM'))
...
class HARAnomalyDetector(_COMNHARAnomalyDetector):
    ...
    """
    def __init__(self, fq=2 * np.pi / 30, fw=2 * np.pi / 7, trend=2):
        super().__init__(fq=fq, fw=fw, trend=trend, methods=('HAR'))
...
class NHARAnomalyDetector(_COMNHARAnomalyDetector):
    ...
    """
    def __init__(self, fq=2 * np.pi / 30, fw=2 * np.pi / 7, trend=2):
        super().__init__(fq=fq, fw=fw, trend=trend, methods=('NHAR'))
